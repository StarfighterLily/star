...
7. **Trait-bounded generics** (`fn f<T: SomeTrait>(x: T)`). Today a generic
   ...
   below for details (the trait-method half; this language has no operator
   overloading at all yet, for generic or concrete types alike, so that half
   of the ceiling is unchanged -- see the note in "Previous work").
...
Implemented trait-bounded generics (todo.md P2 #7): `fn f<T: SomeTrait>(x: T)`, plus the same bound syntax on a generic `struct`/`enum`'s own type parameters (`struct Cage<T: Speaker>: ...`). Only the trait-method half of the original gap is closed — see the note at the end of this entry on the operator half, which is unchanged.
...
- **Not done**: the "or use an operator on it" half of the original gap. This language has no user-defined operator overloading at all yet — `+`/`-`/`*`/`/`/`==`/... are hard-coded to a fixed table of builtin scalar/vector/`Fixed`/time types (`Checker::infer_binop_ty`), with no trait-dispatch mechanism for `+` on a struct regardless of whether it's generic or concrete. Adding one is a materially separate, larger feature (new operator-trait names, checker dispatch, codegen for calling the resolved method in place of a native `add`/`sub`/... instruction) than trait-bounded generics itself, and out of scope for this change.
