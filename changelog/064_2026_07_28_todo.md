...
`star build`-produced executables now link with a generous, explicit stack-size reserve (16MiB) instead of silently inheriting the linker's tiny PE default (1MiB on this project's GNU/mingw target, confirmed empirically) — found via a real user report against `projects/nova`'s binary program loader: a genuine, independently-authored Nova-16 program (`gfxtest.asm`, from the upstream Python repo) loaded and ran for a few thousand steps, then the process died silently with no diagnostic. Bisected to the exact instruction (the program's timer interrupt firing for the first time, whose one-instruction handler is `SROL`) and confirmed via `$LASTEXITCODE` to be `0xC00000FD` (`STATUS_STACK_OVERFLOW`), not a Star-level trap. Root cause: this language's own large-aggregate pointer-passing convention (`Codegen::LARGE_AGGREGATE_THRESHOLD`, todo.md P0 #2) deliberately makes multi-hundred-KB structs/arrays ordinary `let`-bindable values — but every one of those still lives in some function's native stack frame, and `cmd_build` (`src/main.rs`) never told the linker to reserve more than the OS default for that. `projects/nova`'s own `Cpu` (composed of a ~300KB `Memory` and, after this round's 9-layer compositing expansion, a ~650KB `Screen`) held as one `let mut c = Cpu(...)` local in `main`, combined with just one *additional*, unrelated 64KB local array deeper in the same call chain (`Screen::roll_x`'s pre-existing temp buffer), was enough to exceed the 1MiB default — confirmed with a minimal, project-independent repro (a `Cpu`-sized struct in `main` plus one ordinary call to a function with its own 64KB local, no recursion involved). Fixed via a new `stack_size_flag(target)` (`src/main.rs`), added to `cmd_build`'s `clang` invocation alongside the existing `-target`/`-l`/`-L` flags — target-specific, since GNU `ld`/mingw-`lld` (`Target::WindowsGnu`) spells it `--stack,<bytes>` (a PE-header field) while a modern GNU `ld`/`lld` targeting ELF (`Target::LinuxGnu`) spells it `-z stack-size=<bytes>` instead; passing either under the wrong target is a hard link error, not a no-op. Reserved address space costs nothing until touched, so this has no effect on a small program's actual memory footprint. Not the same fix as the pre-existing `MAIN_STACK_SIZE` constant near `fn main` (that one sizes the `star` compiler's *own* process stack for its parser/checker/codegen passes on deep input — a completely different stack from the executables it produces). Verified against the exact repro (now fixed) and the full `cargo test` suite (0 regressions); 2 new tests (`stack_size_flag_uses_windows_ld_stack_spelling`/`_uses_linux_ld_stack_spelling`) in `src/main.rs`'s own `tests` module.
