...
3. **Done.** `readme.md`'s "Versioning" section (commit `bdc4549`,
   pre-dating this pass) already names the gate concretely: "the Nova
   project is complete (full system implementation, GUI+controls, and
   tooling to match Python reference, NoBASIC optional) and minimal 4
   rounds of bug hunts." Judged adequate to close this item — it's a
   fixed, written-down decision with four named components, not a moving
   target that silently grows every time `NOTES.md`'s "Ideas for future
   work" gains an entry, which was this item's actual concern. Not a
   numbered opcode-by-opcode checklist (the item's own "most plausibly"
   framing left that as a suggestion, not a requirement) — a decision at
   this level of concreteness satisfies the underlying ask.
   - One real gap found while checking this for adequacy, now fixed
     rather than left silent: `projects/nova/NOTES.md` states this
     project's own brief as "a **base emulator only**: no assembler, no
     disassembler, no debugger" and separately lists "An
     assembler/disassembler/debugger" under "What's not implemented...
     explicitly out of scope per the brief." Read next to `readme.md`'s
     new "tooling to match Python reference" clause, that's a real
     contradiction if left as-is — a future reader could reasonably
     conclude the language-wide gate is satisfied by a project whose own
     documented brief explicitly excludes the tooling the gate now names.
     Fixed by adding a scope note directly under NOTES.md's brief
     statement (clarifying "per the brief" describes this round's scope,
     not a permanent ceiling) and adding a disassembler/debugger bullet to
     "Ideas for future work" (previously only the assembler was listed
     there) cross-referencing this item. No code changed, no new
     implementation work done — this is a documentation-consistency fix
     only, scoped to what "adequate" actually requires.
   - "GUI+controls" and "tooling to match Python reference" are still
     themselves qualitative, not itemized — genuinely out of scope for
     this item (which only asked for the gate's target to stop moving by
     default, not for every named component to be pre-decomposed into a
     checklist). Whoever eventually works the assembler/disassembler/
     debugger/GUI items should expect to make judgment calls about
     "matches Python reference" scope at that time, the same way any
     other Nova subsystem in this project has been ported by judgment
     rather than by a pre-existing spec.
...
readme.md: not changed this pass — `bdc4549` ("Nova completion criteria
named") had already given the "Versioning" section a concrete gate before
this item was picked up; this pass only judged it adequate rather than
writing it.
projects/nova/NOTES.md: added a scope note under the "base emulator only,
per the brief" line clarifying that the brief describes this round's
scope, not a permanent ceiling, now that `readme.md` names "tooling to
match Python reference" as a gate condition; added a disassembler/debugger
bullet to "Ideas for future work" (previously only the assembler was
listed) so the gate's named components are each traceable to a tracked
future-work item instead of only living in `readme.md`'s prose.
todo.md: P1 #3 marked done with the summary above.
