...
12. **Give the checker/codegen binop-dispatch pattern a real abstraction
    ...
    shared table/trait rather than continuing to hand-add arms. -- Done: See
    previous work below for details.
...
Gave the checker/codegen binop-dispatch pattern a real shared abstraction (todo.md P3 #12), targeting the specific duplication that pattern had actually grown into: `Symbol`, `BitField<N>`, `Flags<E>`, `Color32`, and `PaletteIndex` are all the same shape underneath -- `==`/`!=` only, no ordering, no arithmetic, backed by one opaque-width `icmp` on the type's untagged representation -- but each got its own hand-copied `if` block (five in `Checker::infer_binop_ty`, five more in `Codegen::emit_binop`) as it was added, differing only in the `icmp`'s bit width and the type name baked into the error message. `Wrapping`/`Fixed` (real arithmetic, `%` illegal on `Fixed`) and `Tick`/`Duration`/`Instant` (asymmetric legal pairings) were left as their own dedicated branches/functions -- they already are the "shared abstraction" for their own shape (a `match (lhs,op,rhs)` table for the latter), and folding genuinely different operator semantics into one predicate would only obscure them, exactly what the todo item's own text warned against.

- `src/types/mod.rs`: new `Ty::eq_only_scalar_shape(&self) -> Option<(u32, &'static str)>` (`(bit_width, display_name)`), placed alongside the existing `int_shape`/`bit_shape`/`bitwise_combine_shape` methods it mirrors in shape -- this codebase's own established idiom for "a `Ty -> per-type-constant` table," not a new abstraction style. A future type of this same shape (another packed-bits/interned-id wrapper) needs exactly one new match arm here instead of a new block in each of the two dispatch sites.
- `src/types/expr.rs`'s `Checker::infer_binop_ty` and `src/codegen/vector_math.rs`'s `Codegen::emit_binop`: the five hand-copied blocks in each collapsed into one apiece, driven by the new table. Behavior is unchanged bit-for-bit -- same gating (`lhs_ty == rhs_ty` before the block applies at all, so a same-shape-different-type pair like `Symbol == Color32` or `BitField<8> == BitField<16>` still falls through to the pre-existing generic mismatch diagnostic, not a wrongly-specific one), same error text, same `icmp eq/ne i{width}` shape -- confirmed by the full pre-existing suite passing unchanged after the collapse.
- Added 8 new tests to `tests/frontend.rs`: two per-type coverage gaps this refactor surfaced (`Flags<E>`/`PaletteIndex` ordering and arithmetic rejection had no test at all before, unlike their `Symbol`/`Color32`/`BitField<N>` siblings); two regression tests aimed specifically at the shared table's gating (`BitField<8> == BitField<16>` and `Symbol == Color32` must both still hit the generic mismatch diagnostic, not silently succeed or misreport the same-width-specific message); a codegen-shape test asserting each of the five types' `icmp` uses its own distinct bit width in one pass (`Symbol`/`Flags<E>` -> `i64`, a `BitField<16>` -> `i16` specifically rather than `BitField`'s more commonly-tested `i8`/`i32`, `Color32` -> `i32`, `PaletteIndex` -> `i8`) -- the concrete failure mode a mis-keyed shared table would introduce that five independent hardcoded blocks never could; and a runtime end-to-end companion proving `BitField<64>` compares correctly through the shared path, not just type-checks and emits plausible IR. Full suite (1514 tests) passes.