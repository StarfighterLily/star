...
5. **Done.** The permanent structural caveats — see below for details.
...
6. **Done.** Not an action item — an observation about this cycle's
   trigger, recorded for the historical record. Nothing to implement;
   closed out alongside the rest of the list per the user's direct
   request, since it isn't a "this needs done" line but an encouraging
   note about the process working as intended.
7. **Done.** Standing practice, not an open task — already being
   followed (this cycle and the prior one both started the full `cargo
   test` run before drafting `current_status.md`, not concurrently with
   it). Closed out at the user's direct request rather than left
   perpetually open, since there's no further action pending; the
   practice itself carries forward by convention, not by this line
   staying unchecked.

# Previous work
P2 #5: This item was never an active bug to fix — its own text called it
"not gaps to close, but worth keeping as a standing line item so a future
version decision doesn't have to rediscover them from scratch." Verified
each of the three caveats it names is still true and still just a
documented design choice, not a regression:

1. **"Special guest" types unified in docs, not mechanism** — confirmed by
   rereading `docs/design.md`'s own §2/§4/§6/§8 prose for `Wrapping<T>`,
   `Fixed<Bits,Frac>`, `Tick`/`Duration`/`Instant`, `Symbol`, `Color32`,
   `PaletteIndex`, `BitField<N>`, `Flags<E>`: every one of them is still
   phrased as "gets its own dedicated dispatch branch... instead" of a
   shared mechanism — accurate, unchanged.
2. **Non-dynamic, monomorphized-only traits** — confirmed still current in
   `docs/language_reference.md`'s "Dispatch Model" section (line 444: "no
   `dyn Trait`, no vtable... intentional, permanent design choice") and
   `docs/design.md`'s own "Trait Dispatch: Decision and Scope" section.
3. **Warning-only stack-budget check** — confirmed against
   `src/types/stack_budget.rs`'s module doc comment: still a `star
   check`-time heuristic warning, with `star build`'s 16MiB stack reserve
   as the real backstop, never a hard compile failure.

What was actually missing, and the reason this item kept surfacing cycle
after cycle instead of closing: none of the three had a *permanent*
written home. Each was individually documented in its own scattered
location (fine on its own), but the only place they were ever gathered
into one consolidated "here are the standing caveats" list was
`current_status.md`'s "goals vs. reality" section — which gets archived
to `changelog/` and **overwritten wholesale** every reassessment cycle, so
there was never a stable place a future reader (or a future version-gate
decision) could point to. Fixed by adding a new "Known Permanent Caveats"
section to `docs/design.md` (a durable, load-bearing doc `readme.md`
already points readers to for "the full language specification", unlike
`current_status.md`), consolidating all three with pointers back to each
one's detailed source-of-truth location, plus a note on why the
now-graduated Windows-only-platform-scope caveat isn't a fourth entry
here (it moved to active work under P2 #6 / `docs/cross_platform_scope.md`
on 2026-07-30, per this same file's own prior note). No code changes —
this was a documentation-consolidation task, verified by rereading the
three source-of-truth locations above rather than by any build/test run.
