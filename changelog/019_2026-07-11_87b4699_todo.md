...
### 0. Bugs
- **Closures capturing `self` by pointer can dangle from *any* local struct, not just `frame:`-scoped ones.** `src/types/frame_analysis.rs`'s escape analysis only tracks `let`-bindings declared inside a `frame:` block; an ordinary function-local struct (or a by-value parameter) whose method returns a closure capturing `self` has the exact same problem once that closure escapes the function, since `self` is always passed to methods by pointer (`Codegen::emit_call_expr`) and a closure captures it by pointer too (`Codegen::emit_closure_lit`), not by value. Reproduced: a method returning `fn(): self.val`, called on a plain (non-`frame`) local, then returned out of its enclosing function — prints a garbage value, not a crash, i.e. silent memory corruption. Broadening `frame_analysis` to track every local struct (not just frame ones) is the natural fix given the existing mechanism, but risks over-rejecting sound patterns (e.g. a receiver backed by an arena `GenRef`, which *does* outlive the function) without more careful thought about which receivers are actually safe.
